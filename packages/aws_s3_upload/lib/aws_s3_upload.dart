...
class AwsS3 {
  ...
  static Future<String?> uploadBytes({
    ...
    final endpoint = 'https://$bucket.s3-$region.amazonaws.com';
    String uploadDest = '$destDir/${fileName}';

    if (uploadDest.startsWith('/')) {
      uploadDest = uploadDest.substring(1, uploadDest.length);
    }
    ...
    final multipartBytes = http.MultipartFile.fromBytes('file', bytes,
        filename: fileName, contentType: MediaType("image", "jpg"));

    final policy = Policy.fromS3PresignedPost(
        uploadDest, bucket, accessKey, 15, bytes.length, acl,
        region: region);
    ...
    try {
      ...
      throw Exception(
          'Failed to upload to AWS, with exception: ${e.toString()}');
    }
