...
//    Mat* global_bgra_mat_ptr = nullptr;
    ...
    FUNCTION_ATTRIBUTE
    Mat* convert_camera_image_to_mat( unsigned char *bytes, bool is_yuv, int rotation, int width, int height){
    ...
    FUNCTION_ATTRIBUTE
    uint32_t* convert_yuv_to_rbga_bytes(uint8_t *plane0, uint8_t *plane1, uint8_t *plane2, int bytesPerRow, int bytesPerPixel, int width, int height){
        int hexFF = 255;
...
//                image[getRotatedImageByteIndex(y, x, height)] = (hexFF << 24) | (r << 16) | (g << 8) | b;
    ...
    FUNCTION_ATTRIBUTE
    Mat* convert_camera_image_to_mat_v2(uint8_t *plane0, uint8_t *plane1, uint8_t *plane2, bool is_yuv, int bytesPerRow, int bytesPerPixel, int width, int height){
       ...
       if (is_yuv) {
            rgba_bytes = convert_yuv_to_rbga_bytes( plane0,  plane1,  plane2, bytesPerRow,  bytesPerPixel,  width,  height);
   ...
   FUNCTION_ATTRIBUTE
    unsigned char* convert_camera_image_to_mat_v3(uint8_t *plane0, uint8_t *plane1, uint8_t *plane2, int32_t *imgLengthBytes, bool is_yuv, int bytesPerRow, int bytesPerPixel, int width, int height){
